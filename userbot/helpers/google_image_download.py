...
###### Google Görsellerini yerel diske Arama ve İndirme ######
...
import argparse
import os
import time  # Kod yürütme zamanını kontrol etmek için zaman kitaplığını içe aktarma
import sys
version = (3, 0)
cur_version = sys.version_info
if cur_version >= version:  # Python'un Mevcut Sürümü 3.0 veya üzeri ise
...
def user_input():
    ...
    if object_check['config_file'] != '':
        ...
        parser.add_argument(
            '-k',
            '--keywords',
            help='sınırlandırılmış liste girişi',
            type=str,
            required=False)
        parser.add_argument(
            '-kf',
            '--keywords_from_file',
            help='bir metin dosyasından anahtar kelime listesi çıkar',
            type=str,
            required=False)
        parser.add_argument(
            '-sk',
            '--suffix_keywords',
            help='ana anahtar kelimeden sonra eklenen virgülle ayrılmış ek kelimeler',
            type=str,
            required=False)
        parser.add_argument(
            '-pk',
            '--prefix_keywords',
            help='virgülle ayrılmış ek kelimeler ana anahtar kelimeden önce eklendi',
            type=str,
            required=False)
        parser.add_argument(
            '-l',
        ...
        parser.add_argument(
            '-f',
            '--format',
            help='belirli formattaki resimleri indirin',
            type=str,
            required=False,
            choices=[
                'jpg',
        ...
        parser.add_argument(
            '-wr',
            '--time_range',
            help='görüntünün yaşı için zaman aralığı. formatta olmalı {"time_min":"MM/DD/YYYY","time_max":"MM/DD/YYYY"}',
            type=str,
            required=False)
        parser.add_argument(
            '-a',
            '--aspect_ratio',
            help='Anahtar kelimelere virgülle ayrılmış ek kelimeler eklendi',
            type=str,
            required=False,
            choices=[
                'tall',
        ...
        parser.add_argument(
            '-si',
            '--similar_images',
            help='Sağladığınız görsel URL sine çok benzeyen görselleri indirir',
            type=str,
            required=False)
        parser.add_argument(
            '-ss',
            '--specific_site',
            help='belirli bir web sitesinden indekslenmiş görselleri indirir',
            type=str,
            required=False)
        parser.add_argument(
            '-p',
            '--print_urls',
            default=False,
            help="Görsellerin URL'lerini yazdırın",
            action="store_true")
        parser.add_argument(
            '-ps',
            '--print_size',
            default=False,
            help="Diskteki görüntülerin boyutunu yazdırın",
            action="store_true")
        parser.add_argument(
            '-pp',
            '--print_paths',
            default=False,
            help="Görüntülerin mutlak yollarının listesini yazdırır",
            action="store_true")
        parser.add_argument(
            '-m',
            '--metadata',
            default=False,
            help="Görüntünün meta verilerini yazdırın",
            action="store_true")
        parser.add_argument(
            '-e',
            '--extract_metadata',
            default=False,
            help="Tüm günlükleri bir metin dosyasına döker",
            action="store_true")
        parser.add_argument(
            '-st',
            '--socket_timeout',
            default=False,
            help="Resmin indirilmesini bekleyen bağlantı zaman aşımı",
            type=float)
        parser.add_argument(
            '-th',
            '--thumbnail',
            default=False,
            help="Gerçek görüntü ile birlikte görüntü küçük resmini indirir",
            action="store_true")
        parser.add_argument(
            '-tho',
            '--thumbnail_only',
            default=False,
            help="Gerçek resimleri indirmeden sadece küçük resimleri indirir",
            action="store_true")
        parser.add_argument(
            '-la',
            '--language',
