...
def doit(chat_id, match, original):
    ...
    for f in fl:
        if f == 'i':
            ...
            return None, f"Bilinmeyen bayrak: {f}"
...
CMD_HELP.update({
    "sed":
    ".s <sınırlayıcı> <eski sözcük (ler)> <sınırlayıcı> <yeni sözcük (ler)>\
    \nUsage: Sed kullanarak bir kelimeyi veya kelimeleri değiştirir.\
    \nDelimiters: `/, :, |, _`\
    example: herhangi bir cümleyi etiketleyin ve s / a / b yazın. nerede değiştirilmesi gereken kelime ve b doğru kelimedir."
