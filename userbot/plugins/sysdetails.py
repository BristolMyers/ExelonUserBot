...
from userbot import CMD_HELP, ALIVE_NAME, exelondef, StartTime
import time
# ================= CONSTANT =================
...
#    tempdir = "localdir"
    cmd = "exelon /proc/cpuinfo | grep 'model name'"
#    if dirname == tempdir:
    ...
    o = stdout.decode()
    OUTPUT = f"**[exelon's](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
...
async def sysdetails(sysd):
    try:
        ...
        await sysd.edit("`Hello, on exelonuserbot  install .neofetch first kthx`")
...
async def _(event):
    ...
    OUTPUT = f"**[exelon's](tg://need_update_for_some_feature/) CPU UPTIME:**\n{uptime}"
    await event.edit(OUTPUT)
