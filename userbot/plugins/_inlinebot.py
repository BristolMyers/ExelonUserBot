...
import re
import math
from . import exelonalive
from .. import CMD_LIST
from telethon import Button, custom, events

EXELON_IMG = Config.ALIVE_PIC if Config.ALIVE_PIC else None
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        query = event.text
        if query.startswith(
                "__**ExelonUserBot") and event.query.user_id == bot.uid:
            buttons = [
                (custom.Button.inline(
                    "Durumu", data="stats"), Button.url(
                    "Repo", "https://github.com/BristolMyers/ExelonUserBot"))]
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = "Lütfen kendi Exelonunuzu kurunuz, ve benimkini kullanmayın! yardım için katılın @ExelonUserBot "
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert += "Bu eklentiyi kaldırmak için .unload {} kullanın ©ExelonUserBot".format(
                plugin_name)
            try:
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
            except BaseException:
