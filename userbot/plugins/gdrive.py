...
async def gdrive_upload_function(dryb):
    ...
    if not get_parent_id():
        ...
        exelonparent_id = get_parent_id()
        if len(exelonparent_id) == 1:
            parent_id = exelonparent_id[0].exelon
        elif len(exelonparent_id) > 1:
            for fid in exelonparent_id:
                rmparent_id(fid.exelon)
            parent_id = None
    ...
    if not get_parent_id():
        ...
        if len(exelonparent_id) == 1:
            ...
            for fid in exelonparent_id:
                rmparent_id(fid.exelon)
            parent_id = None
    setf = await exelon.reply("Processing ...")
    input_str = exelon.pattern_match.group(1)
    if input_str:
        gid = input_str
        exelonparent_id = get_parent_id()
        if len(exelonparent_id) == 1:
