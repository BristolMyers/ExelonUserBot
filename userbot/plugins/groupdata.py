...
async def _(event):
    if event.fwd_from:
        return
    mentions = "**Bu Gruptaki Yöneticiler**: \n"
    reply_message = None
    ...
    if not input_str:
        ...
        if not event.is_group:
            await event.edit("Bunun bir grup olduğundan emin misin?")
            return
    else:
        mentions_heading = "{} Gruptaki Yöneticiler: \n".format(input_str)
...
async def get_users(show):
    if show.fwd_from:
        return
    mentions = "**Bu Gruptaki Kullanıcılar**: \n"
    reply_to_id = None
    ...
    if not input_str:
        if not show.is_group:
            await show.edit("Bunun bir grup olduğuna emin misin?")
            return
    else:
        ...
        try:
            ...
            return None
    await show.edit("kullanıcı listesi bekletiliyor...")
    try:
...
async def info(event):
    await event.edit("`Sohbeti analiz etmek...`")
...
async def get_chatinfo(event):
    ...
    try:
        ...
        try:
            ...
            await event.reply("`Bu özel bir kanal / grup veya oradan yasaklandım`")
            return None
        except ChannelPublicGroupNaError:
            await event.reply("`Kanal veya üst grup mevcut değil`")
            return None
...
CMD_HELP.update({
    "groupdata":
    "📌Komut ➥ .chatinfo or .chatinfo <username of group>\
     \nKullanım ➥ Size gerekli sohbetin toplam bilgisini gösterir.\
     \n\n📌Komut ➥ .get_admins or .get_admins <username of group >\
     \nKullanım ➥ Sohbetteki yönetici listesini alır.\
     \n\n📌Komut ➥ .users or .users <name of member>\
