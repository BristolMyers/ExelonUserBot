...
if Config.DUAL_LOG:
    ...
    async def create_dump_channel(event):
        if Config.PM_LOGGR_BOT_API_ID is None:
            result = await event.client(functions.channels.CreateChannelRequest(  # pylint:disable=E0602
                title=f"Exelonuserbot-{borg.uid}-PM_LOGGR_BOT_API_ID-data",
            ...
            with io.BytesIO(str.encode(str(created_chat_id))) as out_file:
                ...
                await event.client.send_file(
                    ...
                    force_document=True,
                    allow_cache=False,
                    caption=f"Lütfen `PM_LOGGR_BOT_API_ID` öğesini {created_chat_id} olarak ayarlayın",
                    reply_to=1
                )
            await event.delete()
        else:
            await event.edit(f"**yapılandırılmıştır **. [lütfen dokunmayın] (https://t.me/c/ {Config.PM_LOGGR_BOT_API_ID} / 2)")
    ...
    async def on_new_private_message(event):
        ...
        if chat_id == borg.uid:
            # Kaydedilmiş Mesajları kaydetme
            return
        if sender.bot:
            # botları kaydetme
            return
        if sender.verified:
            # doğrulanmış hesapları kaydetme
            return
    ...
    async def on_new_chat_action_message(event):
        ...
        if event.created or event.user_added:
            ...
            if borg.uid in added_by_users:
                ...
                the_message += f"[Kullanıcı](tg://user?id={added_by_user}): `{added_by_user}`\n"
                the_message += f"[Özel Bağlantı](https://t.me/c/{chat_id}/{message_id})\n"
    ...
    async def on_new_channel_message(event):
        ...
        if isinstance(event, types.UpdateChannel):
            ...
            the_message += f"[Özel Bağlantı](https://t.me/c/{channel_id}/{message_id})\n"
    ...
    async def do_log_pm_action(chat_id, event, message_text, message_media):
        ...
        the_message += f"Mesaj: {message_text}\n"
