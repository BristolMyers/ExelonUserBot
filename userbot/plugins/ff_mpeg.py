    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if event.reply_to_msg_id:
            ...
            try:
                ...
                await edit_or_reply(event, "{} Saniye içinde `{}` klasörüne indirildi.".format(downloaded_file_name, ms))
        else:
            await edit_or_reply(event, "Telegram medya dosyasını yanıtlayın")
    else:
        await edit_or_reply(event, f"yolda zaten bir medya dosyası var. Lütfen medyayı çıkarın ve tekrar deneyin!\n`.exec rm {FF_MPEG_DOWN_LOAD_MEDIA_PATH}`")
...
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        await edit_or_reply(event, f"bir medya dosyasının indirilmesi ve aşağıdaki yola kaydedilmesi gerekir: `{FF_MPEG_DOWN_LOAD_MEDIA_PATH}`")
        return
    ...
    ms = (end - start).seconds
    await event.edit(f"İşlemi tamamlandı {ms} saniye")
